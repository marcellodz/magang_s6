...
function safeQuery($conn, $query) { // Hapus `: bool|mysqli_result`
    try {
        return mysqli_query($conn, $query);
    } catch (\mysqli_sql_exception $e) { // Tambahkan backslash untuk kompatibilitas
        return false;
    }
}
...
switch ($type) {
    ...
    // =================== QR CODE ===================
case 'qr':
    $sql = "SELECT iduser, nama FROM super_user WHERE iduser='$iduser'";
    ...
    case 'scan':
        $data = safeQuery($conn2, "
            ...
            WHERE iduser='$iduser' GROUP BY idbooth
        ...
        echo "<div class='card'>
            ...
            <p style='line-height:40px'>Lakukan scan QR di setiap booth untuk mendapatkan hadiah menarik!</p>
    ...
    document.addEventListener('click', async function(e) {
        ...
        try {
            const res = await fetch('generate_qr_claim');
