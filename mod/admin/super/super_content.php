...
switch ($type) {
  ...
  case 'peserta':
    ...
    //          kelas AS 'Profesi / Kelas', sekolah AS 'Asal Sekolah', sekolah_lainnya AS 'Sekolah Lainnya', provinsi AS 'Provinsi', kota AS 'Kota',
    ...
    //button export peserta
      echo "
    <form method='post' action='super/export_peserta.php' style='margin:10px 0 20px 0;'>
      <button type='submit' class='btn-add' style='background:linear-gradient(90deg,#0f9d58,#0b8043);'>
...
if ($type === 'kegiatan_peserta') {
  echo "
    ...
    <div id='kegiatan-pagination' class='pagination' style='margin-top:20px;'></div>
    ";
} else {
...
if (isset($totalPage) && $totalPage > 1): ?>
  <div class="pagination">
    <?php for ($i = 1; $i <= $totalPage; $i++):
      ...
      ?>
      <button class="page-btn <?= $active ?>" data-page="<?= $i ?>" data-type="<?= $type ?>">
        <?= $i ?>
      </button>
    <?php endfor; ?>
  </div>
